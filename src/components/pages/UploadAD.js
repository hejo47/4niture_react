...
const UploadAD = () => {
  ...
  return (
    <>
      ...
      >
        <Form labelCol={{ span: 4 }} wrapperCol={{ span: 14 }} layout='horizontal' style={{ maxWidth: 800 }} name='productUpload' onFinish={onFinish}>
          <Form.Item label='상품이름' name='name' rules={[{ required: true, message: "상품이름을 입력해주세요" }]}>
            <Input />
          </Form.Item>
          <Form.Item label='상품가격' name='price' rules={[{ required: true, message: "가격을 입력해주세요 " }]}>
            <Input />
          </Form.Item>
          <Form.Item label='Category' name='category' rules={[{ required: true, message: "카테고리를 선택해주세요" }]}>
            <Select>
          ...
          <Form.Item label='상품이미지' valuePropName='image' size='large'>
            <Upload name='상품이미지' action={`${API_URL}/image`} listType='picture-card' maxCount={2} onChange={pathImage} multiple>
              <div>
                <PlusOutlined />
